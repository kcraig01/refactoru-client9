...
// a= ['a','b','c','d','e']  splice 2,2

var splice = function(arr, start, lastnum, insert) {
	var newArray=[];
	var endRemove = start + lastnum;
	
			for (i=0; i<arr.length; i++) {
				if (typeof insert !== "undefined" && i === start)  
				{
					console.log("i got to here!");
					newArray.push(insert);
					console.log(newArray);				
				}
				if((i<start) || (i>=endRemove)) 
				{
					newArray.push(arr[i]);
					console.log(newArray);
			...
			if (start>arr.length){
				for (i = arr.length; i<arguments.length;i++)
